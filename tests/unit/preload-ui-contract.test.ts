 ...
 *
 * Verifies that every `window.neonPost.*` call in the UI HTML files
...
function parsePreloadAPI(source: string): Set<string> {
  ...
  const cbMatch = source.match(
    /contextBridge\.exposeInMainWorld\(\s*['"]neonPost['"]\s*,\s*\{/
  );
...
/**
 * Extract all `window.neonPost.X.Y` paths from HTML source.
 * Returns unique dotted paths (without 'window.neonPost.' prefix).
...
function extractUIAPICalls(html: string): Set<string> {
  ...
  const regex =
    /window\.neonPost\??\.([a-zA-Z_$][a-zA-Z0-9_$]*)\??\.([a-zA-Z_$][a-zA-Z0-9_$]*)/g;
  let match;
...
describe('Preload ↔ UI API Contract', () => {
  ...
  for (const htmlFile of htmlFiles) {
    it(`${htmlFile}: all neonPost calls match preload API`, () => {
      ...
      if (missingPaths.length > 0) {
        expect.fail(
          `${htmlFile} references ${missingPaths.length} API path(s) not in preload:\n` +
            missingPaths.map((p) => `  • window.neonPost.${p}`).join('\n') +
  ...
  // Detect stale flat API calls (pre-namespace era: window.neonPost.send())
  for (const htmlFile of htmlFiles) {
    it(`${htmlFile}: no flat (non-namespaced) neonPost calls`, () => {
      const html = fs.readFileSync(path.join(uiDir, htmlFile), 'utf-8');

      // Match window.neonPost.X( where X is NOT a known namespace
      // This catches old-style flat calls like window.neonPost.send(...)
      ...
      const flatCallRegex = /window\.neonPost\.([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\(/g;
      ...
      if (flatCalls.length > 0) {
        ...
        expect.fail(
          `${htmlFile} uses ${unique.length} flat (non-namespaced) neonPost call(s):\n` +
            unique.map((n) => `  • window.neonPost.${n}()`).join('\n') +
            '\n\nThese should be updated to use the namespaced API (e.g. window.neonPost.agent.send())'
        );
      }
    });
  }
