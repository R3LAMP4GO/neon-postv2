...
async function stopQuery() {
  ...
  try {
    await window.neonPost.agent.stop(sessionId);
  } catch (err) {
...
async function sendMessage() {
  ...
  try {
    ...
    const result = await window.neonPost.agent.send(
      fullMessage,
      sessionId,
...
async function buildMessageWithAttachments(message, attachments) {
  ...
  for (const att of attachments) {
    if (att.isImage && att.dataUrl) {
      ...
      try {
        const filePath = await window.neonPost.attachments.save(att.name, att.dataUrl);
      ...
      try {
        const filePath = await window.neonPost.attachments.save(att.name, att.dataUrl);
        try {
          const text = await window.neonPost.attachments.extractText(filePath);
...
async function clearChat() {
  ...
  try {
    await window.neonPost.agent.clearConversation(currentSessionId);
