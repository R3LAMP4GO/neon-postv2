...
async function initAgentMode() {
  try {
    ...
    const mode = await window.neonPost.agent.getSessionMode(currentSessionId);
  ...
  window.neonPost.agent.onModeChanged((mode) => {
  ...
  window.neonPost.agent.onSessionModeChanged((sessionId, mode) => {
...
async function setAgentMode(mode) {
  ...
  try {
    const result = await window.neonPost.agent.setSessionMode(currentSessionId, mode);
    if (!result.success) {
      console.warn('Cannot change session mode:', result.error);
      // Revert UI to actual session mode
      const actualMode = await window.neonPost.agent.getSessionMode(currentSessionId);
  ...
  window.neonPost.agent.setMode(mode).catch(err => {
...
async function updateModeUIForSession(sessionId) {
  try {
    const mode = await window.neonPost.agent.getSessionMode(sessionId);
    currentAgentMode = mode || 'coder';
    ...
    const select = document.getElementById('mode-select');
    if (select) {
      const history = await window.neonPost.agent.getHistory(1, sessionId);
