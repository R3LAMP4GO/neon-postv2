...
async function loadSessions() {
  try {
    sessions = await window.neonPost.sessions.list();
...
async function createNewSession() {
  ...
  try {
    const result = await window.neonPost.sessions.create(getNextSessionName());
...
async function finishRename(sessionId, newName) {
  ...
  try {
    const result = await window.neonPost.sessions.rename(sessionId, sanitizedName);
    if (!result.success) {
...
async function deleteSession(sessionId) {
  ...
  try {
    ...
    const result = await window.neonPost.sessions.delete(sessionId);
