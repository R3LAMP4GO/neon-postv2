...
async function _pzLoadAgentName() {
  try {
    const name = await window.neonPost.settings.get('personalize.agentName');
    document.getElementById('pz-agent-name-input').value = name || 'Frankie';
    const desc = await window.neonPost.settings.get('personalize.description');
...
async function pzSaveAgentName() {
  ...
  try {
    await window.neonPost.settings.set('personalize.agentName', name);
    await window.neonPost.settings.set('personalize.description', desc);
...
async function _pzLoadPersonality() {
  try {
    const p = await window.neonPost.settings.get('personalize.personality');
...
async function pzSavePersonality() {
  try {
    await window.neonPost.settings.set('personalize.personality', document.getElementById('pz-personality-editor').value);
    _pzShowToast('Saved! Reboot to apply', 'success');
    _pzActivateReboot();
...
async function _pzLoadWorld() {
  _pzInitWorldTabs();
  try {
    document.getElementById('pz-world-goals').value = await window.neonPost.settings.get('personalize.goals') || '';
    document.getElementById('pz-world-struggles').value = await window.neonPost.settings.get('personalize.struggles') || '';
    document.getElementById('pz-world-funfacts').value = await window.neonPost.settings.get('personalize.funFacts') || '';
  } catch (e) { console.error('[Personalize] Error loading world:', e); }
}

async function pzSaveWorld() {
  try {
    await window.neonPost.settings.set('personalize.goals', document.getElementById('pz-world-goals').value);
    await window.neonPost.settings.set('personalize.struggles', document.getElementById('pz-world-struggles').value);
    await window.neonPost.settings.set('personalize.funFacts', document.getElementById('pz-world-funfacts').value);
    _pzShowToast('Saved! Reboot to apply', 'success');
    _pzActivateReboot();
...
async function _pzLoadSystemPrompt() {
  try {
    const [content, modes] = await Promise.all([
      window.neonPost.customize.getSystemPrompt(),
      window.neonPost.customize.getAgentModes(),
    ]);
...
async function _pzLoadTimezones() {
  try {
    const timezones = await window.neonPost.location.getTimezones();
...
async function _pzLoadProfile() {
  try {
    for (const field of _pzProfileFields) {
      const value = await window.neonPost.settings.get(`profile.${field}`);
      const input = document.getElementById(`pz-profile-${field}`);
      if (input) input.value = value || '';
    }
    const birthday = await window.neonPost.settings.get('profile.birthday');
    if (birthday) {
...
async function pzSaveProfile() {
  ...
  try {
    for (const field of _pzProfileFields) {
      const input = document.getElementById(`pz-profile-${field}`);
      if (input) await window.neonPost.settings.set(`profile.${field}`, input.value);
    }
    ...
    await window.neonPost.settings.set('profile.birthday', month && day ? `${month} ${day}` : '');
