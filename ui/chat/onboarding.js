...
async function checkAndShowOnboarding() {
  try {
    const isFirstRun = await window.neonPost.settings.isFirstRun();
  ...
  const platform = window.neonPost.app.getPlatform();
...
async function obInitKeychain() {
  ...
  try {
    const result = await window.neonPost.settings.initializeKeychain();
    if (result.available) {
...
async function obCheckAndShowPermissions() {
  try {
    const mac = await window.neonPost.permissions.isMacOS();
    if (!mac) {
...
async function obRefreshPermissions() {
  ...
  try {
    const statuses = await window.neonPost.permissions.check([
      'full-disk-access',
      'accessibility',
      'screen-recording',
    ]);
...
async function obOpenPermSettings(type) {
  try {
    await window.neonPost.permissions.openSettings(type);
  } catch (err) {
...
async function obCompleteOAuth() {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (kimiKey) await window.neonPost.settings.set('moonshot.apiKey', kimiKey);
      const glmKey = document.getElementById('ob-glm-key-oauth').value.trim();
      if (glmKey) await window.neonPost.settings.set('glm.apiKey', glmKey);
      const orKey = document.getElementById('ob-openrouter-key-oauth').value.trim();
      if (orKey) await window.neonPost.settings.set('openrouter.apiKey', orKey);
...
async function obCancelOAuth() {
  try {
    await window.neonPost.auth.cancelOAuth();
  } catch (err) {
...
async function obValidateAndSave() {
  ...
  try {
    if (anthropicKey) {
      const result = await window.neonPost.validate.anthropicKey(anthropicKey);
      if (!result.valid) {
    ...
    if (anthropicKey) await window.neonPost.settings.set('anthropic.apiKey', anthropicKey);
    if (kimiKey) await window.neonPost.settings.set('moonshot.apiKey', kimiKey);
    if (glmKey) await window.neonPost.settings.set('glm.apiKey', glmKey);
    if (openrouterKey) await window.neonPost.settings.set('openrouter.apiKey', openrouterKey);

    // Auto-select matching model if default doesn't match available keys
    const currentModel = await window.neonPost.settings.get('agent.model');
    const isAnthropicModel = !currentModel || currentModel.startsWith('claude-');
    if (isAnthropicModel && !anthropicKey) {
      if (openrouterKey) {
        await window.neonPost.settings.set('agent.model', 'moonshotai/kimi-k2.5');
      } else if (kimiKey) {
        await window.neonPost.settings.set('agent.model', 'kimi-k2.5');
      } else if (glmKey) {
        await window.neonPost.settings.set('agent.model', 'glm-4.7');
      }
    }
...
async function obFinishSetup() {
  ...
  try {
    ...
    await window.neonPost.settings.set('onboarding.completed', 'true');
...
async function obSaveName() {
  ...
  if (value) await window.neonPost.settings.set('profile.name', value);
...
async function obSaveLocation() {
  ...
  if (value) await window.neonPost.settings.set('profile.location', value);
  if (timezone) await window.neonPost.settings.set('profile.timezone', timezone);
...
async function obSaveOccupation() {
  ...
  if (value) await window.neonPost.settings.set('profile.occupation', value);
...
async function obSaveBirthday() {
  ...
  if (month && day) await window.neonPost.settings.set('profile.birthday', `${month} ${day}`);
...
async function obSaveAgentName() {
  ...
  if (value) await window.neonPost.settings.set('personalize.agentName', value);
  obShowStep('ob-step-goals');
}

async function obSaveGoals() {
  const value = document.getElementById('ob-goals-input').value.trim();
  if (value) await window.neonPost.settings.set('personalize.goals', value);
...
async function obSaveStruggles() {
  ...
  if (value) await window.neonPost.settings.set('personalize.struggles', value);
  obShowStep('ob-step-funfacts');
}

async function obSaveFunFacts() {
  const value = document.getElementById('ob-funfacts-input').value.trim();
  if (value) await window.neonPost.settings.set('personalize.funFacts', value);
...
function _obSetupLocationAutocomplete() {
  ...
  input.addEventListener('input', (e) => {
    ...
    _obLocationLookupTimeout = setTimeout(async () => {
      try {
        const results = await window.neonPost.location.lookup(query);
...
const _obCliIsWindows = typeof window.neonPost?.app?.getPlatform === 'function' && window.neonPost.app.getPlatform() === 'win32';
...
async function obInstallCli() {
  ...
  try {
    await window.neonPost.shell.runCommand(_obCliCommands.install);
