...
async function _rtnLoadSessions() {
  try {
    const sessions = await window.neonPost.sessions.list();
...
async function _rtnLoadWorkflows() {
  try {
    _rtnWorkflowCommands = await window.neonPost.commands.list();
...
async function _rtnLoadJobs() {
  ...
  try {
    const allJobs = await window.neonPost.cron.list();
...
async function rtnCreateJob() {
  ...
  try {
    const result = await window.neonPost.cron.create(name, schedule, prompt, 'default', sessionId);
    if (result.success) {
...
async function rtnToggleJob(name, enabled) {
  try {
    await window.neonPost.cron.toggle(name, enabled);
...
async function rtnRunJob(name) {
  ...
  try { await window.neonPost.cron.run(name); }
  catch (err) { _rtnShowToast(err.message, 'error'); }
}
...
async function rtnDeleteJob(name) {
  ...
  try {
    await window.neonPost.cron.delete(name);
