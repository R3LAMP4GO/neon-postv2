async function toggleWorkflows() {
  ...
  if (panel.classList.contains('open') && !workflowsPanel.classList.contains('hidden')) {
    ...
    try {
      const commands = await window.neonPost.commands.list(currentSessionId);
...
async function getOrCreateChatUsername() {
  // Load admin key
  try {
    chatAdminKey = (await window.neonPost.settings.get('chat.adminKey')) || '';
    // Admin key loaded
  } catch (e) { console.log('[Chat] Admin key load failed:', e); chatAdminKey = ''; }

  try {
    const existing = await window.neonPost.settings.get('chat.username');
    if (existing) {
      const normalized = existing.toLowerCase();
      globalChatUsername = normalized;
      // Fix any stored mixed-case usernames
      if (normalized !== existing) {
        try { await window.neonPost.settings.set('chat.username', normalized); } catch {}
      }
      return normalized;
    }
  ...
  let firstName = 'anon';
  try {
    const name = await window.neonPost.settings.get('profile.name');
  ...
  try {
    await window.neonPost.settings.set('chat.username', username);
  } catch { /* best effort */ }
  ...
  chatWs.onmessage = (event) => {
    ...
    if (data.type === 'joined') {
      ...
      if (oldName === globalChatUsername) {
        ...
        try { window.neonPost.settings.set('chat.username', newName); } catch {}
        updateHeaderTierBadge();
      }
...
const CHAT_ALLOWED_LINKS = [
  ...
  /https?:\/\/(www\.)?github\.com\/creativeprofit22(\/\S*|$)?(?![a-z0-9-])/i,
];
