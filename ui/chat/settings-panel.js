...
function _initSettingsPanel() {
  ...
  root.addEventListener('click', (e) => {
    ...
    if (link && link.href && (link.target === '_blank' || link.href.startsWith('http'))) {
      e.preventDefault();
      window.neonPost.app.openExternal(link.href);
    }
  });
...
async function _stgLoadAppVersion() {
  try {
    const version = await window.neonPost.app.getVersion();
...
async function _stgLoadSettings() {
  try {
    _stgSettings = await window.neonPost.settings.getAll();
    _stgPopulateFields();
    _stgUpdateToggles();
...
async function _stgRefreshModelDropdown() {
  try {
    const models = await window.neonPost.settings.getAvailableModels();
    const dropdown = document.getElementById('agent.model');
    if (!dropdown) return;
    const savedModel = _stgSettings['agent.model'] || await window.neonPost.settings.get('agent.model');
    ...
    if (isValidSelection) {
      ...
      await window.neonPost.settings.set('agent.model', models[0].id);
    }
  } catch (err) {
...
function _stgSetupNavigation() {
  ...
  if (window.neonPost?.events?.onModelChanged) {
    window.neonPost.events.onModelChanged(() => _stgRefreshModelDropdown());
  }
}
...
function _stgSetupAutoSave() {
  ...
  inputs.forEach(input => {
    ...
    input.addEventListener('change', async () => {
      ...
      try {
        await window.neonPost.settings.set(key, value);
...
async function stgSaveKey(inputId) {
  ...
  try {
    await window.neonPost.settings.set(inputId, key);
...
async function stgDeleteKey(keyId, inputId) {
  ...
  try {
    await window.neonPost.settings.set(keyId, '');
    _stgSettings[keyId] = '';
...
async function stgToggleSetting(key) {
  ...
  try {
    await window.neonPost.settings.set(key, newValue);
...
async function stgValidateKey(provider) {
  ...
  try {
    let result;
    if (provider === 'anthropic') result = await window.neonPost.validate.anthropicKey(key);
    else if (provider === 'openai') result = await window.neonPost.validate.openAIKey(key);
    else if (provider === 'moonshot') result = await window.neonPost.validate.moonshotKey(key);
    else if (provider === 'glm') result = await window.neonPost.validate.glmKey(key);
    else if (provider === 'openrouter') result = await window.neonPost.validate.openRouterKey(key);
    else if (provider === 'telegram') result = await window.neonPost.validate.telegramToken(key);
...
async function stgRestartAgent() {
  ...
  try {
    ...
    await window.neonPost.agent.restart();
...
async function stgSaveTelegramSetting(inputId) {
  ...
  try {
    await window.neonPost.settings.set(inputId, value);
...
async function stgToggleiOS() {
  ...
  try {
    await window.neonPost.settings.set('ios.enabled', newValue.toString());
    _stgSettings['ios.enabled'] = newValue.toString();
    _stgUpdateToggles();
    const result = await window.neonPost.ios.toggle(newValue);
    if (result.success) {
...
async function _stgRefreshPairingCode(regenerate) {
  try {
    const result = await window.neonPost.ios.getPairingCode(!!regenerate);
...
async function _stgRefreshIOSInfo() {
  try {
    const info = await window.neonPost.ios.getInfo();
...
async function _stgRefreshConnectedDevices() {
  try {
    const devices = await window.neonPost.ios.getDevices();
...
async function stgSaveChatUsername() {
  ...
  try {
    ...
    input.value = raw;
    await window.neonPost.settings.set('chat.username', raw);
...
async function stgSaveChatAdminKey() {
  try {
    ...
    await window.neonPost.settings.set('chat.adminKey', adminKey);
...
async function _stgUpdateAuthStatus() {
  ...
  if (authMethod === 'oauth' && hasOAuth) {
    ...
    window.neonPost.auth.validateOAuth().then(result => {
...
async function stgStartOAuth() {
  ...
  try {
    const result = await window.neonPost.auth.startOAuth();
    if (result.success) {
...
async function stgCompleteOAuth() {
  ...
  try {
    const result = await window.neonPost.auth.completeOAuth(code);
    if (result.success) {
...
async function stgSaveApiKey() {
  ...
  try {
    const result = await window.neonPost.validate.anthropicKey(key);
    if (result.valid) {
      await window.neonPost.settings.set('anthropic.apiKey', key);
      await window.neonPost.settings.set('auth.method', 'api_key');
...
async function stgLogout() {
  ...
  try {
    await window.neonPost.settings.set('auth.method', '');
    await window.neonPost.settings.set('auth.oauthToken', '');
    await window.neonPost.settings.set('auth.refreshToken', '');
    await window.neonPost.settings.set('auth.tokenExpiresAt', '');
    await window.neonPost.settings.set('anthropic.apiKey', '');
...
const _STG_CLI_IS_WINDOWS = typeof window.neonPost?.app?.getPlatform === 'function' && window.neonPost.app.getPlatform() === 'win32';
...
const _stgCliCommands = {
  ...
  fetchLatest: _STG_CLI_IS_WINDOWS
    ? 'Invoke-RestMethod https://api.github.com/repos/creativeprofit22/neon-postv2/releases/latest | ConvertTo-Json -Depth 10'
    : 'curl -fsSL https://api.github.com/repos/creativeprofit22/neon-postv2/releases/latest',
  install: _STG_CLI_IS_WINDOWS
    ? [
        '$installDir = Join-Path $env:LOCALAPPDATA "neon-post-cli"',
        'New-Item -ItemType Directory -Force -Path $installDir | Out-Null',
        '$release = Invoke-RestMethod "https://api.github.com/repos/creativeprofit22/neon-postv2/releases/latest"',
    ...
    : 'curl -fsSL https://raw.githubusercontent.com/creativeprofit22/neon-postv2/main/scripts/install.sh -o /tmp/pocket-cli-install.sh && sed -i "" "s/^.*exec .*$//" /tmp/pocket-cli-install.sh && bash /tmp/pocket-cli-install.sh && rm -f /tmp/pocket-cli-install.sh',
};
...
async function _stgInitPocketCli() {
  ...
  try {
    const result = await window.neonPost.shell.runCommand(_stgCliCommands.which);
    if (result && result.trim()) {
      const versionCmd = _stgCliCommands.version(result.trim());
      if (versionCmd) {
        try {
          const versionResult = await window.neonPost.shell.runCommand(versionCmd);
...
async function stgCheckPocketCliUpdates() {
  ...
  try {
    const latestJson = await window.neonPost.shell.runCommand(_stgCliCommands.fetchLatest);
    if (latestJson) {
...
async function stgInstallPocketCli() {
  ...
  try {
    ...
    await window.neonPost.shell.runCommand(_stgCliCommands.install);
...
async function _stgInitializeBrowserSection() {
  ...
  try {
    const browsers = await window.neonPost.browser.detectInstalled();
...
async function stgLaunchBrowserWithCdp() {
  ...
  try {
    const result = await window.neonPost.browser.launch(browserId, port);
    if (result.success) {
      ...
      await window.neonPost.settings.set('browser.cdpUrl', cdpInput.value);
...
async function stgTestBrowserConnection() {
  ...
  try {
    const result = await window.neonPost.browser.testConnection(cdpUrl);
    if (result.connected) {
      statusEl.className = 'status success'; statusEl.textContent = 'Connected';
...
function _stgInitializeUpdates() {
  ...
  if (window.neonPost?.updater?.onStatus) {
    _stgUpdateStatusCleanup = window.neonPost.updater.onStatus(_stgHandleUpdateStatus);
  }
  if (window.neonPost?.updater?.getStatus) {
    window.neonPost.updater.getStatus().then(_stgHandleUpdateStatus).catch(() => {});
  }
}
...
async function stgCheckForUpdates() {
  ...
  try { await window.neonPost.updater.checkForUpdates(); }
...
async function stgDownloadUpdate() {
  ...
  try { const result = await window.neonPost.updater.download(); if (!result.success) _stgShowToast(result.error || 'Download failed', 'error'); }
...
async function stgInstallUpdate() {
  ...
  try { await window.neonPost.updater.install(); }
...
async function _stgInitSkinPicker() {
  try {
    _stgThemesCache = await window.neonPost.themes.list();
    _stgCurrentSkinId = await window.neonPost.themes.getSkin();
    _stgRenderSkinGrid();
    window.neonPost.themes.onSkinChanged((skinId) => {
...
async function _stgSelectSkin(skinId) {
  ...
  _stgRenderSkinGrid();
  _stgApplyTheme(skinId);
  await window.neonPost.settings.set('ui.skin', skinId);
}
