...
export function registerMiscIPC(deps: IPCDependencies): void {
  ...
  const ALLOWED_COMMAND_PREFIXES = IS_WINDOWS
    ? [
        '(Get-Command pocket',
        'Invoke-RestMethod https://api.github.com/repos/creativeprofit22/',
        '$installDir = Join-Path',
      ]
    : [
        'which pocket',
        'curl -fsSL https://api.github.com/repos/creativeprofit22/neon-postv2/',
        'curl -fsSL https://raw.githubusercontent.com/creativeprofit22/neon-postv2/main/scripts/install.sh -o /tmp/pocket-cli-install.sh && sed',
      ];
